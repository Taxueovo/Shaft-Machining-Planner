...
def _get_finish_operation(feature_type: str) -> str:
    ...
    return {
        "keyway": "Precision grind keyway",
        "hole": "Ream hole",
        "flat": "Grind flat",
        "thread": "Thread grinding",
        "bearing_seat": "Precision grind bearing seat",
        "spline": "Precision grind spline",
        "taper": "Precision grind taper",
        # 以车代磨: after quenching, seal areas are hard-turned with CBN instead of ground (cnc_machining.md);
        # bearing seats keep grinding.
        "seal_area": "Finish hard turn seal area",
        "gear_teeth": "Precision grind gear teeth",
        "bore": "Finish bore",
...
def _hard_finish_process(feature_type: str) -> Optional[str]:
    ...
    return {
        ...
        "spline": "Gear Grinding",
        "taper": "Cylindrical Grinding",
        # 以车代磨: quenched seal areas are hard-turned (ISO Turning / CBN), not ground.
        "seal_area": "ISO Turning",
        "gear_teeth": "Gear Grinding",
        "bore": "Boring",
        "cam": "Cam Grinding",
        "worm": "Worm Grinding",
        "crank_pin": "Cylindrical Grinding",
    }.get(feature_type)
...
def build_route(
    ...
    blank_desc = (
        ...
        if is_hollow
        else f"bar stock, total length {geometry['total_length_mm']} mm"
    )
    add_operation(
        operations, "Blanking", "blank", f"Cut from {blank_desc}, reserve face allowance.", None
    )
    add_operation(
        operations,
        "Face Turning",
        "datum",
        f"Turn both faces to establish axial datum.{material_notes}",
        "ISO Turning",
    )
    add_operation(
        operations,
        "Center Drilling",
        "datum",
        "Drill center holes at both ends for center clamping.",
        "Drilling",
    )
    add_operation(
        operations,
        "Rough Turning",
        "rough",
        f"Rough turn stepped profile with allowance (rough allowance ~{rough_allowance}).{material_notes}",
        "ISO Turning",
    )
    ...
    add_operation(
        ...
        "Semi-finish Turning",
        "semi_finish",
        f"Semi-finish turn segments with finishing allowance (finish allowance ~{finish_allowance}).{material_notes}",
        "ISO Turning",
    )
    ...
    for feature in geometry["features"]:
        ...
        needs_pre_heat = (
            ...
            or (high and has_heat and timing == "before_heat_treatment")
        )
        ...
        add_operation(
            ...
            _get_pre_heat_process(feature_type),
            feature_id,
            True,
        )
        if is_split:
    ...
    if has_heat:
        ...
        if heat_plan.get("requires_datum_recovery", True):
            add_operation(
                ...
                "Repair Center Holes",
                "datum_recovery",
                "Recover finishing datum after heat treatment.",
                None,
            )
    ...
    add_operation(
        ...
        "Finish Turning",
        "finish",
        "Finish turn the full stepped profile to target dimensions, including fillets, chamfers and end tapers.",
        "ISO Turning",
    )
    ...
    if grinding_segments:
        ...
        add_operation(
            ...
            "Finish Grind OD",
            "precision_finish",
            "High-precision segment grinding: "
            + ", ".join(grinding_segments)
    ...
    for feature in geometry["features"]:
        ...
        if feature_type == "gear_teeth" and feature.get("gear_finish_required"):
            ...
            description = (
                f"{feature_id} {FEATURE_NAME[feature_type]}, ensure input tolerance and roughness."
            )
        add_operation(
            operations,
            operation_name,
    ...
    for feature in geometry["features"]:
        ...
        if feature_type in pre_heat_features:
            ...
            operation_name, process = _get_post_finish_operation(
                feature_type, feature["high_precision"]
            )
        add_operation(
            operations,
            operation_name,
    ...
    straighten_desc = (
        ...
        if is_slender
        else "Straighten if distortion exceeds tolerance after machining/heat treatment."
    )
    add_operation(
        operations,
        "Straighten",
    ...
    add_operation(
        ...
        "Chamfer & Deburr",
        "deburr",
        "Chamfer sharp edges and deburr all machined features (fillets, end bevels, edges) per drawing.",
        None,
    )
    ...
    add_operation(
        ...
        "Final Inspection",
        "inspection",
        "Check segment dimensions, feature positions, tolerance, roughness and appearance.",
        None,
    )
    ...
    add_operation(
        ...
        "Cleaning",
        "packaging",
        "Clean part to remove cutting fluid, chips and contaminants.",
        None,
    )
    add_operation(
        operations,
        "Packaging",
        "packaging",
        "Apply rust preventive oil and package for shipment.",
        None,
    )
...
def _build_carburized_gear_shaft_route(
    ...
    add_operation(
        ...
        "Blanking",
        "blank",
        f"Cut from purchased {blank_desc}, reserve machining allowance.",
        None,
    )
    add_operation(
        operations,
    ...
    add_operation(
        ...
        "Semi-finish Turning",
        "semi_finish",
        f"Semi-finish turn segments with allowance (soft state).{material_notes}",
        "ISO Turning",
    )
    add_operation(
        operations,
    ...
    add_operation(
        ...
        "Clean Quench Oil",
        "heat_treatment",
        "Clean quench oil residue from the part after quenching.",
        None,
        conditional=True,
    )
    add_operation(
    ...
    add_operation(
        ...
        "Shot Peening",
        "precision_finish",
        "Shot peen cylindrical surfaces to induce compressive residual stress.",
        None,
        conditional=True,
    )
    for feature in geometry["features"]:
    ...
    add_operation(
        ...
        "Laser Marking",
        "feature_before_inspection",
        "Laser-mark part number, drawing and batch identifiers.",
        None,
        conditional=True,
    )
    add_operation(
        ...
        "inspection",
        "MPI inspection of ground surfaces for grinding cracks and defects.",
        None,
        conditional=True,
    )
    add_operation(
    ...
    add_operation(
        ...
        "packaging",
        "Clean part and apply rust preventive oil.",
        None,
    )
    add_operation(operations, "Packaging", "packaging", "Package for shipment.", None)
...
def _append_soft_features(
    ...
    for feature in geometry["features"]:
        ...
        add_operation(
            operations,
            _get_feature_operation(feature_type, is_split),
            "feature_before_heat",
            f"{feature_id} {FEATURE_NAME[feature_type]}, {_feature_position_desc(feature)}; "
            + ("Reserve finishing allowance." if is_split else "Process to input dimensions."),
            _get_pre_heat_process(feature_type),
...
def _append_hard_finish_features(
    ...
    for feature in geometry["features"]:
        ...
        add_operation(
            operations, operation_name, "feature_after_heat", description, process, feature_id, True
        )
...
def _build_camshaft_route(
    ...
    add_operation(
        ...
        "Blanking",
        "blank",
        f"Cut from {blank_desc}, reserve machining allowance.",
        None,
    )
    add_operation(
        operations,
    ...
    add_operation(
        ...
        "Rough Turning",
        "rough",
        f"Rough turn journals and cam profile with allowance.{material_notes}",
        "ISO Turning",
    )
    add_operation(
        operations,
    ...
    add_operation(
        ...
        "Semi-finish Turning",
        "semi_finish",
        f"Semi-finish turn journals and cam profile with allowance.{material_notes}",
        "ISO Turning",
    )
    ...
    add_operation(
        ...
        "Polish",
        "feature_after_heat",
        "Polish journal and cam surfaces to required finish.",
        None,
        conditional=True,
    )
    ...
    add_operation(
        ...
        "Chamfer & Deburr",
        "deburr",
        "Chamfer sharp edges and deburr all machined features per drawing.",
        None,
    )
    if surface != "none":
        add_operation(
...
def _build_crankshaft_route(
    ...
    add_operation(
        ...
        "Face Turning",
        "datum",
        f"Mill/turn both end faces to establish axial datum.{material_notes}",
        "ISO Turning",
    )
    add_operation(
        operations,
    ...
    add_operation(
        ...
        "Rough Turning",
        "rough",
        f"Rough turn main journals (turn-turn-broach / CNC external mill).{material_notes}",
        "ISO Turning",
    )
    if crank_fid:
        add_operation(
    ...
    add_operation(
        ...
        "Semi-finish Turning",
        "semi_finish",
        f"Semi-finish turn journals and crank pins with finishing allowance.{material_notes}",
        "ISO Turning",
    )
    ...
    add_operation(
        ...
        "Dynamic Balancing",
        "feature_before_inspection",
        "Dynamic balance the crankshaft to the required balance grade.",
        None,
    )
...
def _build_worm_shaft_route(
    ...
    add_operation(
        ...
        "Semi-finish Turning",
        "semi_finish",
        f"Semi-finish turn external profile with allowance.{material_notes}",
        "ISO Turning",
    )
    if worm_fid:
        add_operation(
    ...
    if heat == "carburize_quench":
        ...
        add_operation(
            ...
            "Lapping Center Holes",
            "feature_after_heat",
            "Re-lap center holes before final grinding.",
            None,
            conditional=True,
        )
        add_operation(
            ...
            "feature_after_heat",
            "Semi-finish grind external profile.",
            "Cylindrical Grinding",
            conditional=True,
        )
        if worm_fid:
...
def _build_surface_hardened_shaft_route(
    ...
    add_operation(
        ...
        "Semi-finish Grind OD",
        "precision_finish",
        "Semi-finish grind external profile after hardening.",
        "Cylindrical Grinding",
        conditional=True,
    )
    add_operation(
    ...
    add_operation(
        ...
        "Lapping & Polishing",
        "feature_before_inspection",
        "Lap and polish precision surfaces to final finish.",
        None,
        conditional=True,
    )
