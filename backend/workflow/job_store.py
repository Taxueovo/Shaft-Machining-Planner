...
class JobStore:
    ...
    def __init__(self) -> None:
        ...
        self.db_path = configured if configured == ":memory:" else str(Path(configured).expanduser().resolve())
        ...
        self.connection.execute("CREATE TABLE IF NOT EXISTS jobs (job_id TEXT PRIMARY KEY, payload TEXT NOT NULL, updated_at TEXT NOT NULL)")
    ...
    def _persist(self, job_id: str) -> None:
        payload = json.dumps(self.jobs[job_id], ensure_ascii=False, default=str, separators=(",", ":"))
    ...
    def _cleanup_old_jobs(self) -> None:
        completed = [(jid, job) for jid, job in self.jobs.items() if job.get("status") in {"completed", "resource_mismatch", "failed"}]
        ...
        self.connection.executemany("DELETE FROM jobs WHERE job_id = ?", [(job_id,) for job_id in ids])
        self.connection.commit()
    ...
    def stats(self) -> dict[str, int]:
        with self.lock:
            active = sum(job.get("status") in {"running", "queued", "waiting_user_choice"} for job in self.jobs.values())
