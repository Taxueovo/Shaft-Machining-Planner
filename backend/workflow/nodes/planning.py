...
class PlanningNodesMixin:
    ...
    def task_planning(self, state: WorkflowState) -> dict[str, Any]:
        ...
        subtasks: list[dict[str, Any]] = [
            {
                ...
                "description": "Calculate segment coordinates and feature positions.",
                "agent": "feature_analysis",
                "priority": 1,
            },
            {
                "id": "ST02",
                "name": "Heat treatment decision",
                "description": "Apply material and requirement knowledge to determine heat-treatment constraints.",
                "agent": "heat_treatment_planning",
                "priority": 2,
            },
        ]
        ...
        if needs_hitl:
            subtasks.append(
                {
                    "id": "ST03",
                    "name": "Precision choice",
                    "description": "User selects processing timing for high-precision features.",
                    "agent": "precision_choice",
                    "priority": 3,
                }
            )
        ...
        subtasks.extend(
            [
                {
                    "id": f"ST{planning_start:02d}",
                    "name": "Process route generation",
                    "description": "Generate process route based on features and requirements.",
                    "agent": "process_planning",
                    "priority": planning_start,
                },
                {
                    "id": f"ST{planning_start + 1:02d}",
                    "name": "Resource matching",
                    "description": "Query machine/tool database and match per operation.",
                    "agent": "resource_selection",
                    "priority": planning_start + 1,
                },
                {
        ...
        if global_req["surface_treatment"] != "none":
            summary_parts.append(
                f"Surface Treatment: {SURFACE_NAME[global_req['surface_treatment']]}"
            )
        ...
        for source in request.get("features", []):
            ...
            if item["positioning_mode"] == "segment_relative":
                ...
                if index > len(segments):
                    raise ValueError(
                        f"{item['feature_id']} references non-existent segment {index}."
                    )
                ...
                if position > total:
                    raise ValueError(
                        f"{item['feature_id']} global position exceeds total part length."
                    )
                segment = next(
            ...
            if (
                item["high_precision"]
                and request["global_requirements"]["heat_treatment"] == "none"
            ):
                warnings.append(
                    f"{item['feature_id']} is high-precision but no heat treatment; system will arrange after OD finishing."
                )
    ...
    def heat_treatment_planning(self, state: WorkflowState) -> dict[str, Any]:
        ...
        self.progress(
            state,
            20,
            "heat_treatment_planning",
            "Evaluating heat treatment requirements and constraints.",
        )
