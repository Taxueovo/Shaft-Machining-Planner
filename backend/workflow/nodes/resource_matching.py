...
class SelectionNodesMixin:
    ...
    def resource_selection(self, state: WorkflowState) -> dict[str, Any]:
        self.progress(state, 75, "resource_selection", "Querying resources and matching per operation.")

        request, geometry = state["request"], state["geometry"]

        # -- Query machines --
        machine = self.machine_repo.search_turning(geometry["total_length_mm"], float(request["blank_diameter_mm"]))
        ...
        if any(f["feature_type"] == "knurl" for f in geometry["features"]):
            notes.append("Knurl tool specs not covered in current tool material table, needs engineer confirmation.")
        ...
        tool_critical_ok = tool_checks["ISO Turning"]["conclusion"] == "satisfied"
        if not machine_ok:
            notes.append(
        ...
        for operation in state["process_route"]:
            ...
            if process is None:
                status, recommendations, machine_recommendations, note = (
                    ResourceStatus.not_applicable.value, [], [], "No tool/equipment verification needed."
                )
                ...
                status, recommendations, machine_recommendations, note = (
                    ResourceStatus.not_covered.value, [], [], "Current Excel does not cover this process."
                )
            if status not in (ResourceStatus.satisfied.value, ResourceStatus.not_applicable.value):
                partial += 1
            operation_resources.append({
                "operation_no": operation["operation_no"], "operation_name": operation["name"],
                "process_category": process, "verification_status": status,
        ...
        if llm_available() and operation_resources:
            try:
                self.store.update(state["job_id"], current_step="resource_selection",
                                  message="Ranking machines/tools with AI...")
                llm_analysis = self._llm_resource_ranking(state, operation_resources, machine_candidates, tool_calls)
    ...
    def _llm_resource_ranking(self, state: WorkflowState, operation_resources: list[dict[str, Any]], machine_candidates: list[dict[str, Any]], tool_calls: list[dict[str, Any]]) -> dict[str, Any]:
        machine_desc = "\n".join(
        ...
        op_desc = "\n".join(
            ...
            + (f" Recommended: {', '.join(t['cutting_tool_grade'] for t in r['tool_recommendations'][:3])}" if r['tool_recommendations'] else "")
        ...
        messages = self.prompt_manager.render_messages("resource_ranking", {
            "machine_desc": machine_desc, "op_desc": op_desc,
