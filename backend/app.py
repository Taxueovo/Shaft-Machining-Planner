# 提供后端 HTTP 接口，负责本地令牌鉴权、规划任务、资源预览和案例查询。
...
# Surface an LLM misconfiguration loudly instead of silently degrading to rules.
warn_if_llm_misconfigured()
...
# 校验特权接口的本地令牌，并限制请求体大小，避免绕过前端直接调用。
@app.middleware("http")
...
# 返回服务健康状态及可用性信息，不向调用方暴露本地敏感路径。
@app.get("/health")
async def health() -> dict[str, Any]:
    checks = {
        "machine_db": MACHINE_FILE.is_file(),
        "tool_db": TOOL_FILE.is_file(),
...
# 根据当前表单输入预览路线并匹配资源，返回可解释的警告和能力结论。
...
def preview_route(request_data: dict[str, Any]) -> dict[str, Any]:
    ...
    # 把预览输入转换为浮点数；非法值记录警告并使用默认值。
    def _num(value, default: float = 0.0, label: str = "value") -> float:
        """Coerce a preview input to float, warning instead of crashing on bad input."""
        try:
            return float(value)
        except (TypeError, ValueError):
            warnings.append(f"Invalid numeric value for {label}: {value!r}; using {default}.")
            return default

    # 复用数值转换逻辑，将预览字段转换成整数。
    def _int(value, default: int = 1, label: str = "value") -> int:
        try:
            return int(value)
        except (TypeError, ValueError):
            warnings.append(f"Invalid integer value for {label}: {value!r}; using {default}.")
            return default

    blank_dia = _num(request_data.get("blank_diameter_mm", 50), 50.0, "blank_diameter_mm")

    if not segments_raw:
        return {"route": [], "warnings": ["Please add at least one segment."] + warnings}
    ...
    for source in segments_raw:
        ...
        cursor += _num(item.get("length_mm", 0), 0.0, f"{item.get('segment_id')} length_mm")
    ...
    for source in features_raw:
        ...
        if item.get("positioning_mode") == "segment_relative":
            index = _int(item.get("segment_index", 1), 1, f"{item.get('feature_id')} segment_index")
            if index > len(segments) or index < 1:
                warnings.append(
                    f"{item.get('feature_id', '?')} references non-existent segment {index}."
                )
                continue
            segment = segments[index - 1]
            offset = _num(
                item.get("segment_offset_mm", 0), 0.0, f"{item.get('feature_id')} segment_offset_mm"
            )
            if offset > float(segment["length_mm"]):
                warnings.append(
                    f"{item.get('feature_id', '?')} offset exceeds segment {index} length."
                )
                continue
            ...
            position = _num(
                item.get("global_position_mm", 0),
                0.0,
                f"{item.get('feature_id')} global_position_mm",
            )
            segment = next(
                (s for s in segments if s["global_start_mm"] <= position <= s["global_end_mm"]),
                None,
            )
    ...
    geo = {
        ...
        "max_finished_diameter_mm": max(
            (
                _num(s.get("diameter_mm"), 0.0, f"{s.get('segment_id')} diameter_mm")
                for s in segments
            ),
            default=0,
        ),
    ...
    process_categories = sorted(
        {operation["process_category"] for operation in route if operation.get("process_category")}
    )
    ...
    required_module = max(
        [
            _num(feature.get(key), 0.0, f"{feature.get('feature_id')} {key}")
    ...
    if any(feature.get("feature_type") == "knurl" for feature in features):
        resource_notes.append(
            "Knurl tool specs are not covered in the current tool material table; engineer confirmation is required."
        )
    ...
    capability = {
        ...
        "overall": ResourceStatus.satisfied.value
        if critical_ok
        ...
        operation_resources.append(
            {
                ...
                "recommendation_provenance": {
                    ...
                    if any(item.get("unverified_constraints") for item in machine_recommendations)
    ...
    if not critical_ok:
        resource_notes.append(
            "Critical turning capability is not satisfied by the active local machine and tool libraries."
        )
    ...
    materials = [
        {
            ...
            "label": "45 Steel",
            "category": "P",
            "description": "Quality carbon structural steel, most common motor shaft material",
        },
        {
            "value": "40Cr",
            "label": "40Cr",
            "category": "P",
            "description": "Alloy structural steel, good comprehensive properties after quench and temper",
        },
        {
            "value": "42CrMo",
            "label": "42CrMo",
            "category": "P",
            "description": "High-strength alloy steel, first choice for heavy-duty shafts",
        },
        {
            "value": "35CrMo",
            "label": "35CrMo",
            "category": "P",
            "description": "Medium-carbon alloy steel, high fatigue strength",
        },
        {
            "value": "20Cr",
            "label": "20Cr",
            "category": "P",
            "description": "Carburizing steel, used for camshafts and wear-resistant parts",
        },
        {
            "value": "20CrMnTi",
            "label": "20CrMnTi",
            "category": "P",
            "description": "Carburizing steel, hard surface tough core",
        },
        {
            "value": "Q235",
            "label": "Q235",
            "category": "P",
            "description": "Plain carbon steel, low-cost light-duty shaft",
        },
        {
            "value": "45Mn2",
            "label": "45Mn2",
            "category": "P",
            "description": "Quenched and tempered steel, high-strength medium-duty shaft",
        },
        {
            "value": "303",
            "label": "303 Stainless Steel",
            "category": "M",
            "description": "Free-machining austenitic stainless steel, excellent machinability",
        },
        {
            "value": "304",
        ...
        },
        {
            ...
            "category": "M",
            "description": "Acid-alkali resistant, chemical equipment shaft",
        },
        {
            "value": "2Cr13",
            "label": "2Cr13",
            "category": "M",
            "description": "Martensitic stainless steel, can be strengthened by heat treatment",
        },
        {
            "value": "1Cr17Ni2",
            "label": "1Cr17Ni2",
            "category": "M",
            "description": "High-strength stainless steel",
        },
        {
            "value": "GCr15",
            "label": "GCr15",
            "category": "H",
            "description": "High-carbon chromium bearing steel, high hardness high wear resistance",
        },
        {
            "value": "GCr15SiMn",
            "label": "GCr15SiMn",
            "category": "H",
            "description": "Large section bearing steel",
        },
        {
            "value": "6061",
            "label": "6061 Aluminum",
            "category": "N",
            "description": "Lightweight shaft, easy to cut",
        },
        {
            "value": "7075",
            "label": "7075 Aluminum",
            "category": "N",
            "description": "High-strength aluminum alloy, aerospace shaft",
        },
        {
            "value": "H62",
            "label": "H62 Brass",
            "category": "N",
            "description": "Easy to cut, corrosion resistant",
        },
    ]
    return {"materials": materials}


# 按输入几何和材料参数返回成本估算，不表示实际采购报价。
@app.get("/api/v1/materials/price")
def get_material_price(
    material: str,
    blank_type: str = "solid",
    outer_dia: float = 50,
    inner_dia: float = 0,
    length: float = 100,
) -> dict[str, Any]:
    ...
    return estimate_material_cost(
        ...
        blank_type,
        outer_dia,
        inner_dia if inner_dia > 0 else None,
        length,
    )
...
# 按工具名称和参数调用注册工具，将无效请求转换成接口错误。
@app.post("/api/v1/tools/{tool_name}")
def call_tool(tool_name: str, params: dict[str, Any]) -> dict[str, Any]:
    try:
        return {
            "tool": tool_name,
            "result": service.workflow.tool_registry.call(tool_name, **params),
        }
    except ValueError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except Exception as error:
        raise HTTPException(
            status_code=500, detail=f"Tool execution failed: {type(error).__name__}: {error}"
        ) from error


# 返回已登记智能体的名称及能力声明，供状态界面查看。
...
def orchestrator_status() -> dict[str, Any]:
    wf = service.workflow
    return {
        "orchestrator": wf.orchestrator.get_status(),
        "tools": wf.tool_registry.list_tools(),
        "guardrails_rules": len(wf.guardrails._rules),
    }
...
def get_taxonomy_node(node_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "children": [n.model_dump() for n in taxonomy_db.get_children(node_id)],
        "path": [n.model_dump() for n in taxonomy_db.get_path(node_id)],
    }


# 按指定分类及其后代筛选案例，保留分页语义。
@app.get("/api/v1/taxonomy/{node_id}/cases")
def get_taxonomy_cases(
    node_id: str, industry: Optional[str] = None, material: Optional[str] = None
) -> dict[str, Any]:
    node = taxonomy_db.get_node(node_id)
    if not node:
        raise HTTPException(status_code=404, detail="Taxonomy node not found.")
